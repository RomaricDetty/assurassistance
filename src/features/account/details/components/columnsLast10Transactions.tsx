...
const TransactionDetailsModal = ({
  ...
  const [transactionData, setTransactionData] = useState<
    TransactionColumnSchema & { cashier: any; receiver: any }
  >();
...
export const columnsLast10Transactions: ColumnDef<TransactionColumnSchema>[] = [
  {
    accessorKey: "type",
    header: ({ column }) => (
      <DataTableColumnHeader column={column} title="Opération" />
    ),
    enableHiding: false,
    cell: ({ row }) => {
      const typeOperation = row.getValue("type") as string;
      ...
      return (
        <Badge variant={variant} className="max-w-[200px]">
          {renderTransactionType(typeOperation)}
        </Badge>
      );
    },
  },
